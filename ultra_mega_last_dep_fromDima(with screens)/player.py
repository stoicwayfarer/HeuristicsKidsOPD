...
class Player(pygame.sprite.Sprite, Collisions):
    ...
    def update(self, dt, tiles, enemies):
        ...
        if self.position.y > SCREEN_HEIGHT:
            self.hp -= 1
            death_sound = pygame.mixer.Sound('Sounds/Death_player_sound.mp3')
            death_sound.play()
            if self.hp > 0:
                self.respawn()
                return
...
class Enemy(pygame.sprite.Sprite, Collisions):
    def __init__(self, x, y, speed=1, patrol_distance=150):
        ...
        self.wall_check_distance = 10  # Distance to check for walls
        self.alive = True
        self.FACING_LEFT = False  # Added for consistent facing direction
    ...
    def update(self, dt, player, tiles):
        ...
        if self.initialized and self.on_ground:
            self.check_for_obstacles(tiles)  # Combined check for ledges and walls
    ...
    def check_for_obstacles(self, tiles):
        # Check for ledge
        ...
        # Check for wall
        wall_check_x = self.rect.left - self.wall_check_distance if self.direction == -1 else self.rect.right + self.wall_check_distance
        wall_check_rect = pygame.Rect(wall_check_x, self.rect.top, 1, self.rect.height)
        ...
        # Change direction if either obstacle is detected
        if not has_ground or has_wall:
            self.direction *= -1
            self.FACING_LEFT = self.direction == -1  

    # Удален метод respawn(), так как враги больше не возрождаются
    ...
    def draw(self, display, camera):
        if self.alive:
            display.blit(self.image, camera.apply(self))