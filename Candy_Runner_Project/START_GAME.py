...
class Level:
    def __init__(self, level_number, map_file, enemy_positions):
        ...
        self.score = 0
        self.timer = 60
        self.points_to_next_level = 1100
    ...
    def next_level(self):
        if self.level_number + 1 < len(levels_data):
            ...
            self.__init__(
                self.level_number, 
                levels_data[self.level_number]['map'], 
                levels_data[self.level_number]['enemies']
            )
        else:
    ...
    def draw(self, screen):
        ...
        # вывод параметров
        timer_text = self.font.render(f'Time: {self.timer}', True, BLACK)
        screen.blit(timer_text, (10, 10))
        hp_text = self.font.render(f'HP: {self.player.hp}', True, BLACK)
        screen.blit(hp_text, (10, 40))
        score_text = self.font.render(f'SCORE: {self.score}', True, BLACK)
        screen.blit(score_text, (1345, 10))
        level_text = self.font.render(f'LEVEL: {self.level_number + 1}', True, BLACK)
...
def main():
    ...
    while True:
        ...
        levels_data = [
            {
                'map': 'level_1.csv',
                'enemies': enemy_positions_1
            },
            {
                'map': 'level_2.csv',
                'enemies': enemy_positions_2
            },
            {
                'map': 'level_3.csv',
                'enemies': enemy_positions_3
            }
        ]
